...
class LongshotDataSplitter:
    """`longshot`(単勝>=15倍 かつ 3着内=1/それ以外=0) を目的変数にする Splitter。

    既存の `DataSplitter` / `WinDataSplitter` と同等のインターフェースを持ち、
    既存コードを崩さずに穴馬モデルを追加できるようにする。
    """
