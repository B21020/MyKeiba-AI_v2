...
class StdScorePolicy(AbstractScorePolicy):
    ...
    def calc(model, X: pd.DataFrame) -> pd.DataFrame:
        ...
        z = z.where(np.isfinite(z), s)
        # 全件同一値のグループ（nunique<=1）は元の値を採用
        z = z.where(nuniq > 1, s).fillna(0)
...
class MinMaxScorePolicy(AbstractScorePolicy):
    ...
    def calc(model, X: pd.DataFrame) -> pd.DataFrame:
        ...
        max_ = z.max()
        denom = (max_ - min_)
        score_table[_SCORE] = ((z - min_) / denom).replace([np.inf, -np.inf], 0).fillna(0)
        return score_table
