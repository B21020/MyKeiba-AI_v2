...
def _calc(model, X: pd.DataFrame) -> pd.DataFrame:
    ...
    # 予測入力の作り方を選ぶ
    # - 列名の一致が十分 → feature_names順にreindex
    # - 一致がほぼ無い/少ない → Xの列順を使って numpy 配列で shape だけ合わせる（全0化を避ける）
    use_name_alignment = False
    overlap = 0
    if feature_names is not None:
        try:
            overlap = len(pd.Index(feature_names).intersection(x_cols))
            use_name_alignment = overlap >= max(1, int(0.5 * len(feature_names)))
        except Exception:
            use_name_alignment = False

    if use_name_alignment and feature_names is not None:
        X_model = X.reindex(columns=list(feature_names), fill_value=0)
        X_matrix_mode = False
    else:
        # 列名が合っていない可能性が高い
    ...
    if feature_names is not None and not use_name_alignment:
        warnings.warn(
