...
class FeatureEngineering:
    ...
    def __label_encode(self, target_col: str):
        ...
        # マスタファイルを更新
        combined_master.to_csv(csv_path, index=False)
        
        # エンコーディング辞書を作成（重複を除去）
        encoding_dict = combined_master.drop_duplicates(subset=[target_col]).set_index(target_col)['encoded_id'].to_dict()
        
        # ラベルエンコーディング実行（数値型に変換）
        self.__data[target_col] = self.__data[target_col].map(encoding_dict).astype('float64')
        return self
