...
class FeatureEngineering:
    ...
    def __label_encode(self, target_col: str):
        ...
        # インデックスに重複がないかチェック
        if new_target_master[target_col].duplicated().any():
            print(f"警告: {target_col}に重複があります。さらに除去します。")
            new_target_master = new_target_master.drop_duplicates(subset=[target_col], keep='first')
        
        # Seriesに変換
        id_mapping = new_target_master.set_index(target_col)['encoded_id']
        # マスタファイルを更新
        id_mapping.to_csv(csv_path)
        # ラベルエンコーディング実行
        # NaN値を持つ行を除外してマッピング
        mapped_values = self.__data[target_col].map(id_mapping)
