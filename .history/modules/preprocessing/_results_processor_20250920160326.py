...
class ResultsProcessor(AbstractDataProcessor):
    ...
    def _preprocess(self):
        ...
        # 各列を数値型に変換（errors='coerce'で'---'などを欠損値に変換）
    ...
    def _preprocess_rank(self, raw):
        ...
        # 大量データのprint出力を防ぐため、サマリーのみ表示
        print(f"着順前処理完了: 有効データ {len(df)} 件, 着順範囲 {df[Cols.RANK].min()}-{df[Cols.RANK].max()}")
        return df
