...
class FeatureEngineering:
    ...
    def __label_encode(self, target_col: str):
        ...
        # 元のマスタと繋げる
        if len(new_target) > 0:
            combined_master = pd.concat([target_master, new_target], ignore_index=True)
            # インデックスをリセットしてからset_indexを使用
            combined_master = combined_master.reset_index(drop=True)
        else:
            combined_master = target_master
            
        # 重複を除去してからインデックスを設定
        combined_master = combined_master.drop_duplicates(subset=[target_col])
        new_target_master = combined_master.set_index(target_col)['encoded_id']
        
        # マスタファイルを更新
        new_target_master.to_csv(csv_path)
        # ラベルエンコーディング実行（NaN値はそのまま残す）
