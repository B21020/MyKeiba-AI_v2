...
def get_rawdata_horse_info(html_path_list: list):
    ...
    for html_path in tqdm(html_path_list):
        try:
            ...
            # 馬の基本情報を取得（pd.read_htmlの結果に応じてフォールバック）
            df_info = None
            try:
                dfs = pd.read_html(html)
                if len(dfs) >= 2:
                    # 通常のケース: [1]番目にプロフィール表がある
                    df_info = dfs[1].set_index(0).T
                elif len(dfs) >= 1:
                    # フォールバック: [0]番目を試す
            ...
            # pd.read_htmlで取得できない場合、BeautifulSoupでプロフィール表を手動構築
            if df_info is None or df_info.empty:
                prof_table = soup.find("table", attrs={"summary": "のプロフィール"})
                if prof_table:
                    rows = []
            ...
            if df_info is None or df_info.empty:
                print(f'horse_info empty profile table: {html_path}')
                continue
            ...
            try:
                ...
                trainer_id = re.findall(r"trainer/([\w_\-]+)", trainer_a_list[0]["href"])[0]
            ...
            try:
                ...
                owner_id = re.findall(r"owner/([\w_\-]+)", owner_a_list[0]["href"])[0]
            except (IndexError, AttributeError):
                # 馬主IDを取得できない場合
                owner_id = NaN
            ...
            try:
                ...
                breeder_id = re.findall(r"breeder/([\w_\-]+)", breeder_a_list[0]["href"])[0]
            except (IndexError, AttributeError):
                # 生産者IDを取得できない場合
                breeder_id = NaN
            df_info['breeder_id'] = breeder_id

            # インデックスをhorse_idにする（Windows/Unix両対応）
            path_str = str(html_path)
            horse_id_match = re.findall(r'horse[\\/](\d+)\.bin', path_str)
            if not horse_id_match:
                print(f'horse_id not found in path: {path_str}')
                continue
            horse_id = horse_id_match[0]
