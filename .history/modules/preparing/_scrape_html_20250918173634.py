...
def _merge_results_into_base_html(base_html_text: str, results_fragment_html: str) -> str:
    ...
    print(f"[DEBUG] fragment body exists: {frag_soup.body is not None}")
    
    # lxmlパーサーは自動的にhtml/bodyタグを追加することがあるので、
    # body内のコンテンツを取得する
    if frag_soup.body:
        fragment_content = frag_soup.body.contents
        print(f"[DEBUG] Using body contents, count: {len(fragment_content)}")
    else:
        fragment_content = frag_soup.contents
        print(f"[DEBUG] Using root contents, count: {len(fragment_content)}")
