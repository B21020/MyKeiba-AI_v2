    ...
    try:
        ...
        for tr in driver.find_elements(By.CLASS_NAME, 'HorseList'):
            ...
            for i, td in enumerate(td_elements):
                ...
                # クラス名に基づいてデータをマッピング（より確実な方法）
                if 'Waku' in td_class and 'Txt_C' in td_class:  # 枠番
                    row_data[Cols.WAKUBAN] = td_text
                elif 'Umaban' in td_class and 'Txt_C' in td_class:  # 馬番
                    row_data[Cols.UMABAN] = td_text
                elif 'Barei' in td_class and 'Txt_C' in td_class:  # 性齢
                    row_data[Cols.SEX_AGE] = td_text
                elif td_class == 'Txt_C' and td_text.replace('.', '').isdigit():  # 斤量（数字のみ）
                    if Cols.KINRYO not in row_data:  # まだ設定されていない場合のみ
                        row_data[Cols.KINRYO] = td_text
                elif td_class == 'Txt_R Popular':  # 単勝オッズ
                    row_data[Cols.TANSHO_ODDS] = td_text
                elif 'Popular_Ninki' in td_class and 'Txt_C' in td_class:  # 人気
                    row_data[Cols.POPULARITY] = td_text
                elif td_class == 'Weight':  # 馬体重
        ...
        # データクリーニング - 空文字列やNaNの行を除去
        print(f"クリーニング前: {len(df)}行")
        
        # 必須フィールドが空または無効な行を除去
        mask = (
            (df[Cols.WAKUBAN].astype(str).str.strip() != '') & 
            (df[Cols.UMABAN].astype(str).str.strip() != '') & 
            (df[Cols.SEX_AGE].astype(str).str.strip() != '') & 
            (df[Cols.SEX_AGE] != 'nan') &
            (df['horse_id'].astype(str).str.strip() != '')
        )
